...
// Generate writes an xray-core JSON configuration based on the provided VLESS settings.
// It returns the filename of the generated config.
func Generate(cfg *parser.VLESSConfig) string {
	...
	fileName := "config_" + time.Now().Format("150405") + ".json"
	...
	if err := json.NewEncoder(f).Encode(xrayConf); err != nil {
		log.Fatalf("Error serializing configuration: %v", err)
	}
